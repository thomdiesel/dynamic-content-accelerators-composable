...
(function(exports) {
    ...
    function drawProducts(){
        products.forEach(function(item) {
            var productCode = item.getAttribute('data-amp-product-code');
             $.ajax({
                url: 'https://api.europe-west1.gcp.commercetools.com/anyafinn/product-projections/' + productCode + '?localeProjection=en',
                method: "GET",
                dataType: "json",
                context: item,
                cache: false,
    ...
    function drawDynamicProducts(){
        dynamicProducts.forEach(function(item) {
            ...
            console.log(ignores)
             $.ajax({
                url: 'https://api.europe-west1.gcp.commercetools.com/{CT_PROJECT_KEY/product-projections/search?staged=false&fuzzy=true&limit=' + searchLimit + '&text.en="' + productSearch +'"',
                method: "GET",
                dataType: "json",
                context: item,
                cache: false,
