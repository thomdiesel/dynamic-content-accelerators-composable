...
(function (exports) {
  ...
  function doCountdowns(){
    var counters = document.querySelectorAll('[data-amp-counter]');
    counters.forEach(function (counterdom) {
      var counddowntime = counterdom.getAttribute('data-amp-counter');
      var countDownDate = new Date(JSON.parse(counddowntime)).getTime();
      ...
      var x = setInterval(function() {
        ...
        counterdom.innerHTML = days + "d " + hours + "h "
        + minutes + "m " + seconds + "s ";
          
        // If the count down is over, write some text 
        if (distance < 0) {
          clearInterval(x);
          counterdom.innerHTML = "EXPIRED";
        }
      }, 1000);
